        ...
        <div class="bg-white rounded-lg shadow-md p-8 mb-6">
            ...
            <div class="flex items-center justify-center mb-6">
                @if($testAttempt->candidate->photo)
                    <img src="{{ Storage::url($testAttempt->candidate->photo) }}" alt="{{ $testAttempt->candidate->name }}"
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="text-center p-4 bg-gray-50 rounded-lg">
                    <div
                        class="text-4xl font-bold {{ $testAttempt->percentage >= 80 ? 'text-green-600' : ($testAttempt->percentage >= 60 ? 'text-yellow-600' : 'text-red-600') }}">
                ...
                @foreach($testAttempt->candidateAnswers as $index => $answer)
                    ...
                    <div class="mb-6 pb-6 {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                        <div class="flex items-start mb-3">
                            <span
                                class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center mr-4 {{ $answer->is_correct ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                {{ $answer->is_correct ? '✓' : '✗' }}
                            </span>
                            <div class="flex-1">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        <span
                                            class="ml-2 text-xs px-2 py-1 rounded {{ $answer->is_correct ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                ...
                                @if($question->isMCQ() || $question->isTrueFalse())
                                    <div class="mt-4 space-y-2">
                                        @foreach($question->options as $option)
                                            <div
                                                class="flex items-center p-3 rounded {{ $option->is_correct ? 'bg-green-50 border border-green-200' : ($answer->selected_option_id == $option->id && !$option->is_correct ? 'bg-red-50 border border-red-200' : 'bg-gray-50') }}">
                                                ...
                                                <span
                                                    class="{{ $option->is_correct ? 'font-semibold text-green-900' : ($answer->selected_option_id == $option->id ? 'font-semibold text-red-900' : 'text-gray-700') }}">
                                                    {{ $option->option_text }}
                                                </span>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                @foreach($question->matchingPairs as $pair)
                                                    ...
                                                    @endphp
                                                    <div
                                                        class="flex items-center mb-2 p-2 rounded {{ $isCorrect ? 'bg-green-50' : 'bg-red-50' }}">
                                            ...
                                            <div>
                                                ...
                                                @foreach($question->matchingPairs as $pair)
                                                    <div class="flex items-center mb-2 p-2 bg-green-50 rounded">
                                                        ...
                                                        <span class="mr-2">{{ $pair->column_a_text }}</span>
                                                        <span class="mx-2">→</span>
                                                        <span class="font-semibold text-green-700">{{ $pair->column_b_key }}.
                                                            {{ $pair->column_b_text }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
