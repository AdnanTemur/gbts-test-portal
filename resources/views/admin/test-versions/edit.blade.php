...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Edit Test Version</h1>
    <p class="text-gray-600 mt-1">{{ $testVersion->version_code }}</p>
</div>

<div class="bg-white rounded-lg shadow p-6 max-w-4xl">
    <form action="{{ route('admin.test-versions.update', $testVersion) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Version Code</label>
                <input type="text" value="{{ $testVersion->version_code }}" disabled class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100">
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="description" 
                          name="description" 
                          rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('description', $testVersion->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="shuffle_questions" value="1" {{ old('shuffle_questions', $testVersion->shuffle_questions) ? 'checked' : '' }} class="mr-2">
                    <span class="text-sm text-gray-700">Shuffle questions order</span>
                </label>

                <label class="flex items-center">
                    <input type="checkbox" name="shuffle_options" value="1" {{ old('shuffle_options', $testVersion->shuffle_options) ? 'checked' : '' }} class="mr-2">
