        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    @endphp
                    <span
                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $status['color'] }}">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
                <div class="p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg border border-blue-200">
                    ...
                    <div class="text-2xl font-bold text-blue-900">
                        {{ $testVersion->testSections->sum(fn($s) => $s->pivot->time_limit ?? 0) }} <span
                            class="text-sm">min total</span></div>
                </div>
                ...
                <div class="p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg border border-purple-200">
                    ...
                    <div class="text-xs text-purple-700 mb-1">Total Questions</div>
                    <div class="text-2xl font-bold text-purple-900">
                        {{ $testVersion->testSections->sum(fn($s) => $s->pivot->questions_per_section) }}</div>
                </div>
                ...
                <div class="p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
                    ...
                    <div class="text-2xl font-bold text-green-900">{{ $testVersion->pass_threshold }}<span
                ...
                <div class="p-4 bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg border border-amber-200">
                    ...
                    <div class="text-2xl font-bold text-amber-900">{{ $testVersion->overlap_threshold }}<span
                            class="text-sm">%</span></div>
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-gray-200">
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                        ...
                        <span
                            class="font-semibold text-gray-900 ml-2">{{ $testVersion->shuffle_questions ? 'Yes' : 'No' }}</span>
                    </div>
                    <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                        <svg class="w-4 h-4 text-gray-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <span
                            class="font-semibold text-gray-900 ml-2">{{ $testVersion->shuffle_options ? 'Yes' : 'No' }}</span>
                    </div>
                    <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                        <svg class="w-4 h-4 text-gray-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <span
                            class="font-semibold text-gray-900 ml-2">{{ $testVersion->created_at->format('M d, Y H:i A') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="p-6">
            <div class="space-y-3">
                @foreach($testVersion->testSections as $index => $section)
                    <div
                        class="flex items-center p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg border border-gray-200 hover:border-primary-300 transition-colors">
                        <div
                            class="flex-shrink-0 w-10 h-10 bg-gradient-to-br from-primary-700 to-primary-600 text-white rounded-lg flex items-center justify-center font-bold shadow-sm mr-4">
                            {{ $index + 1 }}
                        </div>
                        <div class="flex-1">
            ...
            @if(count($metrics) > 0)
                <div class="space-y-4">
                    @foreach($metrics as $sectionName => $metric)
                        <div
                            class="p-4 rounded-lg border-2 {{ $metric['is_valid'] ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50' }}">
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <span
                                    class="text-xs font-semibold px-2.5 py-1 rounded-full {{ $metric['is_valid'] ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800' }}">
                                    {{ $metric['message'] }}
                                </span>
                            </div>
                            ...
                            @if(isset($metric['overlap_percentage']))
                                <div class="flex items-center gap-6 text-sm">
                                    <div class="flex items-center">
                                        ...
                                        <span
                                            class="font-bold ml-1.5 {{ $metric['overlap_percentage'] <= $testVersion->overlap_threshold ? 'text-green-700' : 'text-red-700' }}">
                            ...
                            @else
                                <div class="flex items-center text-sm text-gray-700">
                                    ...
                                    <span class="font-semibold">{{ $metric['total_questions'] }}</span> questions available - Unique
                                    sets possible
                                </div>
                            @endif
                        </div>
