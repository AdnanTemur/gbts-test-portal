...
<body>
        ...
        <div class="question-block">
            <div class="question-header">
                <div class="question-number {{ $answer->is_correct ? 'correct' : 'incorrect' }}">
                    Q{{ $questionNumber }}
                    <div style="font-size: 9px;">{{ $answer->is_correct ? '✓' : '✗' }}</div>
                </div>
            ...
            @if($question->isMCQ() || $question->isTrueFalse())
                <div class="options-list">
                    @foreach($question->options as $optIndex => $option)
                        <div class="option-item {{ $option->is_correct ? 'correct' : ($answer->selected_option_id == $option->id ? 'selected-wrong' : 'neutral') }}">
                            <strong>{{ chr(65 + $optIndex) }}.</strong> {{ $option->option_text }}
                            @if($option->is_correct)
                                <strong style="float: right; color: #065f46;">✓ CORRECT ANSWER</strong>
                            @elseif($answer->selected_option_id == $option->id)
                                <strong style="float: right; color: #991b1b;">✗ YOUR ANSWER</strong>
                            @endif
                        </div>
                    @endforeach
            ...
            @if($question->isMatching())
                ...
                @if($answer->is_correct)
                    <div style="padding: 5px; margin-top: 5px; background-color: #d1fae5; color: #065f46; font-weight: bold; text-align: center;">
                        ✓ All pairs matched correctly!
                    </div>
                @else
                    <div style="padding: 5px; margin-top: 5px; background-color: #fee2e2; color: #991b1b; font-weight: bold; text-align: center;">
                        ✗ Not all pairs were matched correctly (0 marks for matching questions)
                    </div>
                @endif
            @endif
        </div>
    ...
    <div class="footer">
        ...
        <div style="margin-top: 10px;">This is a computer-generated answer sheet from the PMA Test Management System</div>
